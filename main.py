...
class DesktopPet(QWidget):
    ...
    def initPall(self):
        #导入准备在托盘显示上使用的图标
        icons = os.path.join('tigerIcon.gif')
        #设置右键显示最小化的菜单项
        #菜单项退出，点击后调用quit函数
    ...
    def initPetImage(self):
        ...
        self.movie = QMovie("normal/everyday1.gif")
        #设置标签大小
        ...
        #调用自定义的randomPositon，会使得宠物出现位置随机
        self.randomPositon()
        #展示
        self.show()
        ...
        for i in os.listdir('normal'):
            self.pet1.append('mormal/' + i)
        #将宠物正常待机状态的对话放入到pet2中
        self.dialog = []
        #读取目录下dialog文件
        ...
        def petNormalAction():
            ...
            self.timer = QTime()
            #时间到了自动执行
            self.timer.timeout.connet(self.randomAct)
            #动作时间切换设置
            self.timer.start(3000)
            #宠物状态设置为正常
            self.condition = 0
            #每隔一段时间切换对话
            self.talkTimer = QTime()
        ...
        def randomAct(self):
            ...
            if not self.condition:
                ...
                #宠物大小
                self.move.setScaledSize(QSize(200,200))
                #将动画添加到label中
                self.image.setMovie(self.movie)
                #开始播放动画
                self.movie.start()
            ...
            def talk(self):
                if not self.talk_condition:
                    ...
                    #根据内容自适应大小
                    self.talkLabel.adhustSize()
                else:
                    #talk_condition为1显示为别点我，这里同样可以通过if-else-if 来拓展对应的行为
                ...
                #显示宠物
                def showwing(self):
                    #setWindowOpacity()设置窗体透明度，通过调整透明度实现宠物的展示和隐藏
                ...
                def mousePressEvent(self, event):
                    ...
                    if event.button() == Qt.LeftButton:
                        self.is_follow_moust = Troshue
                    #globalPos()事件触发点相对于桌面的位置
                    #pos()程序相对于桌面左上角的位置，实际时窗口的左上角坐标
